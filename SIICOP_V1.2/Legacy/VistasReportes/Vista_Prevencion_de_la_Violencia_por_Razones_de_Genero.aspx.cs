...
{
    public partial class Vista_Prevencion_de_la_Violencia_por_Razones_de_Genero : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (User.IsInRole("SysAdmin") || User.IsInRole("Administrador") || User.IsInRole("Cap_PVM") || User.IsInRole("Admin_PVM") || User.IsInRole("Admin_Empresarial"))
                {

                    if (this.User.IsInRole("SYSADMIN") || this.User.IsInRole("Admin_PVM") || this.User.IsInRole("Admin_Empresarial"))
                    {

                        this.GvPVM.Visible = false;
                        this.GvPVMadmin.Visible = true;
                    ...
                    else
                    {
                        ...
                        this.GvPVM.Visible = true;
                        this.GvPVMadmin.Visible = false;
        ...
        protected void conteoactividadesAdmin()
        {
            ...
            if (this.txtFInicialC.Text == string.Empty)
            {
                if (this.iduser != 0)
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.programasID == 10 && x.Personalid == this.iduser).Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.programasID == 10).Count();
            ...
            else
            {
                ...
                if (this.iduser != 0)
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.programasID == 10 && x.Personalid == this.iduser).Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.programasID == 10).Count();
        ...
        protected void GvPVM_DataBound(object sender, EventArgs e)
        {
            try
            {

                // Recupera la el PagerRow...
                GridViewRow pagerRow = GvPVM.BottomPagerRow;
                ...
                if ((pageList != null))
                {
                    ...
                    int i = 0;
                    for (i = 0; i <= GvPVM.PageCount - 1; i++)
                    {
                        // Se crea un objeto ListItem para representar la �gina...
                        int pageNumber = i + 1;
                        ListItem item = new ListItem(pageNumber.ToString());
                        if (i == GvPVM.PageIndex)
                        {
                            item.Selected = true;
                        }
                ...
                if ((pageLabel != null))
                {
                    ...
                    int currentPage = GvPVM.PageIndex + 1;
                    // Actualiza el Label control con la �gina actual.
                    pageLabel.Text = "Página " + currentPage.ToString() + " de " + GvPVM.PageCount.ToString();
        ...
        protected void GvPVM_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Editar")
            {
                ...
                {
                    Session["AccionExpediente"] = PrevenciondelaVM.AccionExpediente.Edicion;
                    Guid idExped = Guid.Parse(numFila.ToString());
                    Session["idExpediente_Accion"] = idExped;
                    Response.Redirect("~/Captura/Prevención_de_la_Violencia_por_Razones_de_Género.aspx");
                }
            }
        ...
        {
            {
                ...
                GvPVM.PageIndex = pageList.SelectedIndex;
                //Quita el mensaje de información si lo hubiera...
                //lblInfo.Text = "";
            }
        }

        protected void GvPVMadmin_DataBound(object sender, EventArgs e)
        {
            try
            {

                // Recupera la el PagerRow...
                GridViewRow pagerRow = GvPVMadmin.BottomPagerRow;
                // Recupera los controles DropDownList y label...
                DropDownList pageList = (DropDownList)pagerRow.FindControl("PageDropDownListAdmin");
                Label pageLabel = (Label)pagerRow.FindControl("CurrentPageLabelr");
                if ((pageList != null))
                {
                    // Se crean los valores del DropDownList tomando el número total de páginas... 
                    int i = 0;
                    for (i = 0; i <= GvPVMadmin.PageCount - 1; i++)
                    {
                        // Se crea un objeto ListItem para representar la �gina...
                        int pageNumber = i + 1;
                        ListItem item = new ListItem(pageNumber.ToString());
                        if (i == GvPVMadmin.PageIndex)
                        {
                            item.Selected = true;
                        }
                        // Se añade el ListItem a la colección de Items del DropDownList...
                ...
                {
                    // Calcula el nº de �gina actual...
                    int currentPage = GvPVMadmin.PageIndex + 1;
                    // Actualiza el Label control con la �gina actual.
                    pageLabel.Text = "Página " + currentPage.ToString() + " de " + GvPVMadmin.PageCount.ToString();
        ...
        protected void GvPVMadmin_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Editar")
            {
                Guid numFila;
        ...
        {
            {
                ...
                DropDownList pageList = (DropDownList)pagerRow.FindControl("PageDropDownListAdmin");
                // Se Establece la propiedad PageIndex para visualizar la página seleccionada...
                GvPVMadmin.PageIndex = pageList.SelectedIndex;
                //Quita el mensaje de información si lo hubiera...
                //lblInfo.Text = "";
            }
        }
        ...
        public static List<datoGraficaPasteNorte> ChartDataNorteAccionesta(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from ar in ctx.tb_Reporte_Diario
                                 ...
                                 where ar.fecha >= fechainicial && ar.fecha <= fechaFinal && xr.RegionID == 1 && ar.programasID == 10
        ...
        public static List<datoGraficaPastecentro> ChartDataCentroAccionesta(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from bc in ctx.tb_Reporte_Diario
                                 ...
                                 where bc.fecha >= fechainicial && bc.fecha <= fechaFinal && nb.RegionID == 2 && bc.programasID == 10
        ...
        public static List<datoGraficabartecentro> datoGraficabarteCentroMuni(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from arx in ctx.tb_Reporte_Diario
                                 ...
                                 where arx.fecha >= fechainicial && arx.fecha <= fechaFinal && xry.RegionID == 2 && arx.programasID == 10
        ...
        public static List<datoGraficaPasteSur> ChartDataSurAccionesta(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from bxc in ctx.tb_Reporte_Diario
                                 ...
                                 where bxc.fecha >= fechainicial && bxc.fecha <= fechaFinal && nb.RegionID == 3 && bxc.programasID == 10
        ...
        public static List<datoGraficabartesur> datoGraficabartesurMuni(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from xxx in ctx.tb_Reporte_Diario
                                 ...
                                 where xxx.fecha >= fechainicial && xxx.fecha <= fechaFinal && rrr.RegionID == 3 & xxx.programasID == 10
        ...
        protected void GvPVM_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void GvPVMadmin_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        private void Conteo_A_B_TB_X_Cargadetrabajo()
        {
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjXAL = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION XALAPA XX" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtXAL.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION XALAPA XX" && s.programasID == 10
            ...
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJXAL = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION XALAPA XX" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjVER = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION VERACRUZ XXIII" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtVER.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION VERACRUZ XXIII" && s.programasID == 10
                                                     select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJVER = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION VERACRUZ XXIII" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjPOZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION POZA RICA" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtPOZA.InnerText = (from s in ctx.tb_Reporte_Diario
                                                      join std in ctx.tb_DireccionReporte
                                                      on s.idResumenDiario equals std.idResumenDiario
                                                      where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION POZA RICA" && s.programasID == 10
                                                      select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJPOZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION POZA RICA" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjCOR = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION CORDOBA" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjTCOR.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "COORDINACION CORDOBA" && s.programasID == 10
                                                     select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJCOR = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION CORDOBA" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjCOATZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION COATZACOALCOS" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtCOATZA.InnerText = (from s in ctx.tb_Reporte_Diario
                                                        join std in ctx.tb_DireccionReporte
                                                        on s.idResumenDiario equals std.idResumenDiario
                                                        where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "COORDINACION COATZACOALCOS" && s.programasID == 10
                                                        select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJCOATZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION COATZACOALCOS" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjENLACE = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "Enlace" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtENLACE.InnerText = (from s in ctx.tb_Reporte_Diario
                                                        join std in ctx.tb_DireccionReporte
                                                        on s.idResumenDiario equals std.idResumenDiario
                                                        where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "ENLACES" && s.programasID == 10
                                                        select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJENLACE = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 10 && x.Mes_Num == mesactual && x.Delegacion == "Enlace" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
