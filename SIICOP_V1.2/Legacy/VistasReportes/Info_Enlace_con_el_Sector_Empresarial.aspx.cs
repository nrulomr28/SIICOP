...
{
    public partial class InfoEmpresarial : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (User.IsInRole("SysAdmin") || User.IsInRole("Administrador") || User.IsInRole("CapSEMP") || User.IsInRole("Admin_Empresarial"))
                {

                    if (this.User.IsInRole("SYSADMIN") || this.User.IsInRole("Admin_Empresarial"))
                    {

                        this.GvSectorEmpreCap.Visible = false;
                        this.GvSEMPAdmin.Visible = true;
                    ...
                    else
                    {
                        ...
                        GvSectorEmpreCap.Visible = true;
                        lkbtnexcel.Visible = false;
                        GvSEMPAdmin.Visible = false;
                        GvSectorEmpreCap.DataBind();
        ...
        protected void conteoactividadesAdmin()
        {
            ...
            if (this.txtFInicialC.Text == string.Empty)
            {
                if (this.iduser != 0)
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.programasID == 13 && x.Personalid == this.iduser).Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.programasID == 13).Count();
            ...
            else
            {
                ...
                if (this.iduser != 0)
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.programasID == 13 && x.Personalid == this.iduser).Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.programasID == 13).Count();
        ...
        protected void lkbtnexcel_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //// consulta de un procedimiento almacenado que jala una vista 
                #region *** descargar excel por medio de fechas
                ...
                //       Teléfono = a.telcel,
                //       Empresa = a.NombreLugar_Escuela,
                //       Descripción = a.descripcion_actividad,
        ...
        //    //        //                                                                          (R.NombrePrograma.Equals(PROGRA) || string.IsNullOrEmpty(PROGRA)))).ToList();
        //    //        List<wv_reportesGeneralExcel> listaReportes = ctx.wv_reportesGeneralExcel.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) &&
        ...
        protected void GvSectorEmpreCap_DataBound(object sender, EventArgs e)
        {
            try
            {

                // Recupera la el PagerRow...
                GridViewRow pagerRow = GvSectorEmpreCap.BottomPagerRow;
                ...
                if ((pageList != null))
                {
                    ...
                    int i = 0;
                    for (i = 0; i <= GvSectorEmpreCap.PageCount - 1; i++)
                    {
                        // Se crea un objeto ListItem para representar la �gina...
                        int pageNumber = i + 1;
                        ListItem item = new ListItem(pageNumber.ToString());
                        if (i == GvSectorEmpreCap.PageIndex)
                        {
                            item.Selected = true;
                        }
                ...
                if ((pageLabel != null))
                {
                    ...
                    int currentPage = GvSectorEmpreCap.PageIndex + 1;
                    // Actualiza el Label control con la �gina actual.
                    pageLabel.Text = "Página " + currentPage.ToString() + " de " + GvSectorEmpreCap.PageCount.ToString();
        ...
        protected void GvSectorEmpreCap_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Editar")
            {
                ...
                if (Guid.TryParse(e.CommandArgument.ToString(), out numFila))
                {
                    ...
                    Response.Redirect("~/Captura/Enlace_con_el_Sector_Empresarial.aspx");
                }
            }
        ...
        {
            {
                ...
                GvSectorEmpreCap.PageIndex = pageList.SelectedIndex;
                //Quita el mensaje de información si lo hubiera...
                //lblInfo.Text = "";
            }
        }

        protected void GvSEMPAdmin_DataBound(object sender, EventArgs e)
        {
            try
            {

                // Recupera la el PagerRow...
                GridViewRow pagerRow = GvSEMPAdmin.BottomPagerRow;
                // Recupera los controles DropDownList y label...
                ...
                if ((pageList != null))
                {
                    // Se crean los valores del DropDownList tomando el número total de páginas... 
                    int i = 0;
                    for (i = 0; i <= GvSEMPAdmin.PageCount - 1; i++)
                    {
                        // Se crea un objeto ListItem para representar la �gina...
                        int pageNumber = i + 1;
                        ListItem item = new ListItem(pageNumber.ToString());
                        if (i == GvSEMPAdmin.PageIndex)
                        {
                            item.Selected = true;
                        }
                        // Se añade el ListItem a la colección de Items del DropDownList...
                ...
                {
                    // Calcula el nº de �gina actual...
                    int currentPage = GvSEMPAdmin.PageIndex + 1;
                    // Actualiza el Label control con la �gina actual.
                    pageLabel.Text = "Página " + currentPage.ToString() + " de " + GvSEMPAdmin.PageCount.ToString();
        ...
        protected void GvSEMPAdmin_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Editar")
            {
                Guid numFila;
        ...
        {
            {
                ...
                DropDownList pageList = (DropDownList)pagerRow.FindControl("PageDropDownListAdmin");
                // Se Establece la propiedad PageIndex para visualizar la página seleccionada...
                GvSEMPAdmin.PageIndex = pageList.SelectedIndex;
                //Quita el mensaje de información si lo hubiera...
                //lblInfo.Text = "";
            }
        }
        ...
        public void totalporZonas()
        {
            ...
            // conteo general de platicas 
            this.HPLATICAS.InnerText = (from s in ctx.tb_Reporte_Diario
                                        where s.fecha >= fechainicial && s.fecha <= fechaFinal && s.AccionesID == 1065 && s.programasID == progragr
                                        select s.AccionesID).Count().ToString();

            // convenios
            this.HCONVENIOS.InnerText = (from s in ctx.tb_Reporte_Diario
                                         where s.fecha >= fechainicial && s.fecha <= fechaFinal && s.AccionesID == 1066 && s.programasID == progragr
                                         select s.AccionesID).Count().ToString();
        ...
        public static List<datoGraficaPasteNorte> ChartDataNorteAccionesta(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from ar in ctx.tb_Reporte_Diario
                                 ...
                                 where ar.fecha >= fechainicial && ar.fecha <= fechaFinal && xr.RegionID == 1 && ar.programasID == 13
        ...
        //}
      //  public static List<datoGraficabarteNorte> datoGraficabarteNorteMuni(string x, string fechaInicial, string fechafinal)
        public static List<datoGraficaPasteNorte> datoGraficabarteNorteMuni(string x, string fechaInicial, string fechafinal)
        {
            // 1. Convertir fechas fuera para mantener el bloque using limpio
            DateTime fInicio, fFin;
            if (!DateTime.TryParse(fechaInicial, out fInicio) || !DateTime.TryParse(fechafinal, out fFin))
            {
                return new List<datoGraficaPasteNorte>(); // Retorna vacío si las fechas están mal
            }

            // 2. Bloque USING para liberar memoria automáticamente al terminar
            using (SIICOPEntities ctx = new SIICOPEntities())
            {
                try
                {
                    // 3. Proyección Directa: Creamos el objeto final en el select
                    // Esto elimina la necesidad del bucle foreach posterior
                    var listaDatos = (from ar in ctx.tb_Reporte_Diario
                                      join bz in ctx.Cat_Acciones on ar.AccionesID equals bz.AccionesID
                                      join xr in ctx.tb_DireccionReporte on ar.idResumenDiario equals xr.idResumenDiario
                                      where ar.fecha >= fInicio
                                            && ar.fecha <= fFin
                                            && xr.RegionID == 1     // OJO: "1" está harcodeado (Norte)
                                            && ar.programasID == 13 // OJO: "13" está harcodeado
                                      group bz by bz.AccionesNombre into g
                                      select new datoGraficaPasteNorte
                                      {
                                          etiquetaR = g.Key,
                                          Cantidad = g.Count()
                                      }).ToList();
                ...
                {
                    // Opcional: Loguear el error aquí (ex.Message)
                    // Retornamos una lista vacía para que la gráfica simplemente no pinte nada,
                    // en lugar de que trone la página.
                    return new List<datoGraficaPasteNorte>();
                }
            }
        }
        ...
        public static List<datoGraficaPastecentro> ChartDataCentroAccionesta(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from bc in ctx.tb_Reporte_Diario
                                 ...
                                 where bc.fecha >= fechainicial && bc.fecha <= fechaFinal && nb.RegionID == 2 && bc.programasID == 13
        ...
        public static List<datoGraficabartecentro> datoGraficabarteCentroMuni(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from arx in ctx.tb_Reporte_Diario
                                 ...
                                 where arx.fecha >= fechainicial && arx.fecha <= fechaFinal && xry.RegionID == 2 && arx.programasID == 13
        ...
        public static List<datoGraficaPasteSur> ChartDataSurAccionesta(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from bxc in ctx.tb_Reporte_Diario
                                 ...
                                 where bxc.fecha >= fechainicial && bxc.fecha <= fechaFinal && nb.RegionID == 3 && bxc.programasID == 13
        ...
        public static List<datoGraficabartesur> datoGraficabartesurMuni(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from xxx in ctx.tb_Reporte_Diario
                                 ...
                                 where xxx.fecha >= fechainicial && xxx.fecha <= fechaFinal && rrr.RegionID == 3 & xxx.programasID == 13
        ...
        protected void GvSEMPAdmin_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
        ...
        protected void GvSectorEmpreCap_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
        ...
        private void Conteo_A_B_TB_X_Cargadetrabajo()
        {
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjXAL = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION XALAPA XX" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtXAL.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION XALAPA XX" && s.programasID == 13
            ...
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJXAL = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION XALAPA XX" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjVER = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION VERACRUZ XXIII" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtVER.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION VERACRUZ XXIII" && s.programasID == 13
                                                     select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJVER = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION VERACRUZ XXIII" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjPOZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION POZA RICA" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtPOZA.InnerText = (from s in ctx.tb_Reporte_Diario
                                                      join std in ctx.tb_DireccionReporte
                                                      on s.idResumenDiario equals std.idResumenDiario
                                                      where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION POZA RICA" && s.programasID == 13
                                                      select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJPOZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION POZA RICA" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjCOR = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION CORDOBA" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjTCOR.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "COORDINACION CORDOBA" && s.programasID == 13
                                                     select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJCOR = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION CORDOBA" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjCOATZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION COATZACOALCOS" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtCOATZA.InnerText = (from s in ctx.tb_Reporte_Diario
                                                        join std in ctx.tb_DireccionReporte
                                                        on s.idResumenDiario equals std.idResumenDiario
                                                        where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "COORDINACION COATZACOALCOS" && s.programasID == 13
                                                        select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJCOATZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION COATZACOALCOS" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjENLACE = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "Enlace" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtENLACE.InnerText = (from s in ctx.tb_Reporte_Diario
                                                        join std in ctx.tb_DireccionReporte
                                                        on s.idResumenDiario equals std.idResumenDiario
                                                        where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "ENLACES" && s.programasID == 13
                                                        select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJENLACE = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 13 && x.Mes_Num == mesactual && x.Delegacion == "Enlace" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
