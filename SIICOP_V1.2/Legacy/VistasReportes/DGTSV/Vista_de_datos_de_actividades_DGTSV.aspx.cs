...
{
    public partial class Vista_de_datos_de_actividades_DGTSV : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (User.IsInRole("SysAdmin") || User.IsInRole("Administrador") || User.IsInRole("DGTSV"))
                {
                ...
                {
                    this.Response.Redirect("~/Bienvenido_DGTSV.aspx");
                }
            }
        }
        ...
        protected void conteoactividadesAdmin()
        {
            ...
            else
            {
                ...
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.Dependencia == "DGTSV").Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => ((x.fecha >= fechainicial && x.fecha <= fechaFinal) && (x.Dependencia == "DGTSV") && (x.RegionID == Zona || x.programasID == tiprograma))).Count();
        ...
        protected void GvPCAdmin_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (valor == 21)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/DGTSV/Pin/Pin.png";
                    }
        ...
        #region ## METODO PARA EDITAR ACTIVIDAD Y VER SI TIENEN FOTOS
        protected void GvPCAdmin_RowCommand(object sender, GridViewCommandEventArgs e)
        {

            int EducacionVial = 21;
            ...
            if (e.CommandName == "Editar")
            {
                ...
                if (EducacionVial == Progrmaid)
                {
                    this.Session["AccionReporte"] = Ficha_idetificacion_diaria.AccionReporte.Edicion;
        ...
        private void LlenaHoja(OfficeOpenXml.ExcelWorksheet xlWorkSheetGral)
        {
            try
            {
                ...
                if (Session["dtPrincipal"] == null)
                {
                    ...
                    if (tiprograma == 0)
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "DGTSV"))).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
                    else
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "DGTSV") && R.programasID == tiprograma)).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
        ...
        public void ExportaImagenesZip()
        {
            ...
            if (programa == 0)
            {
                var consulta = (from Item1 in ctx.tb_Reporte_Diario
                                ...
                                where Item1.fecha >= fechainicial && Item1.fecha <= fechaFinal && Item3.Dependencia == "DGTSV"
                                select new
                                {
