...
namespace SIICOP_V1._2.VistasReportes.CEPREVIDE
{
    public partial class Vista_de_datos_de_actividades_CEPREVIDE : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    this.Response.Redirect("~/Bienvenido_CEPREVIDE.aspx");
                }
            }
        }
        ...
        protected void conteoactividadesAdmin()
        {
            ...
            {
                this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(ñ => ñ.Dependencia == "CEPREVIDE").Count();
            ...
            else
            {
                ...
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.Dependencia == "CEPREVIDE").Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => ((x.fecha >= fechainicial && x.fecha <= fechaFinal) && (x.Dependencia == "CEPREVIDE") && (x.RegionID == Zona || x.programasID == tiprograma))).Count();
        ...
        protected void GvPCAdmin_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (valor == 22)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/CEPREVIDE/Pines/Pin_ParticipaCiudadana.png";
                    }
                    if (valor == 23)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/CEPREVIDE/Pines/Pin_Escolar.png";
                    }
                    if (valor == 24)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/CEPREVIDE/Pines/Pin_GruposVulnerables.png";
                    }
                    if (valor == 25)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/CEPREVIDE/Pines/Pin_CulturaLegalidadyDenuncia.png";
                    }
                    if (valor == 26)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/CEPREVIDE/Pines/Pin_Espacios.png";
                    }
        ...
        #region metodo para Editar desde el grid y ver si tiene foto 
        protected void GvPCAdmin_RowCommand(object sender, GridViewCommandEventArgs e)
        {

            int participacion = 22;
            int escolar = 23;
            int GruposVul = 24;
            int culturaLega = 25;
            int recuperaEspaci = 26;
            ...
            if (e.CommandName == "Editar")
            {
                ...
                if (participacion == Progrmaid)
                {
                    this.Session["AccionReporte"] = Reporte_CEPREVIDE.AccionReporte.Edicion;
                    this.Session["idReporte_Accion"] = (object)idExped;
                    this.Response.Redirect("~/Captura/CEPREVIDE/Reporte_CEPREVIDE.aspx");
                }

                if (escolar == Progrmaid)
                {
                    this.Session["AccionReporte"] = Reporte_CEPREVIDE.AccionReporte.Edicion;
                    this.Session["idReporte_Accion"] = (object)idExped;
                    this.Response.Redirect("~/Captura/CEPREVIDE/Reporte_CEPREVIDE.aspx");
                }
                if (GruposVul == Progrmaid)
                {
                    this.Session["AccionReporte"] = Reporte_CEPREVIDE.AccionReporte.Edicion;
                    this.Session["idReporte_Accion"] = (object)idExped;
                    this.Response.Redirect("~/Captura/CEPREVIDE/Reporte_CEPREVIDE.aspx");
                }
                if (culturaLega == Progrmaid)
                {
                    this.Session["AccionReporte"] = Reporte_CEPREVIDE.AccionReporte.Edicion;
                    this.Session["idReporte_Accion"] = (object)idExped;
                    this.Response.Redirect("~/Captura/CEPREVIDE/Reporte_CEPREVIDE.aspx");
                }
                if (recuperaEspaci == Progrmaid)
                {
                    this.Session["AccionReporte"] = Reporte_CEPREVIDE.AccionReporte.Edicion;
                    this.Session["idReporte_Accion"] = (object)idExped;
                    this.Response.Redirect("~/Captura/CEPREVIDE/Reporte_CEPREVIDE.aspx");
        ...
        private void LlenaHoja(OfficeOpenXml.ExcelWorksheet xlWorkSheetGral)
        {
            try
            {
                ...
                if (Session["dtPrincipal"] == null)
                {
                    ...
                    if (tiprograma == 0)
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "CEPREVIDE"))).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
                    else
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "CEPREVIDE") && R.programasID == tiprograma)).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
        ...
        public void ExportaImagenesZip()
        {
            ...
            if (programa == 0)
            {
                var consulta = (from Item1 in ctx.tb_Reporte_Diario
                                ...
                                where Item1.fecha >= fechainicial && Item1.fecha <= fechaFinal && Item3.Dependencia == "CEPREVIDE"
                                select new
                                {
