...
{
    public partial class Construccion_Cultura_Incluyente : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (User.IsInRole("SYSADMIN") || User.IsInRole("Administrador") || User.IsInRole("Inclusión_Social") || User.IsInRole("Foráneo"))
                {
        ...
        protected void btnGuardarEdicion_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (tbReporteDiario != null)
                {
                    ...
                    if (this.txtHindigenas.Value != "") { IndiH = int.Parse(txtHindigenas.Value); }
                    ...
                    if (this.txtMindigenas.Value != "") { IndiM = int.Parse(txtMindigenas.Value); }
                    int Hombres = nino + padresH + DocentesH + IndiH;
                    int Mujeres = nina + padresM + DocentesM + IndiM;
                ...
                if (datosGralReporte != null)
                {
                    ...
                    // se agregan estos para no ingresar mas datos a la tabla 
                    datosGralReporte.otrosH = new int?(this.txtHindigenas.Value == "" ? 0 : Convert.ToInt32(this.txtHindigenas.Value));
                    datosGralReporte.otrosm = new int?(this.txtMindigenas.Value == "" ? 0 : Convert.ToInt32(this.txtMindigenas.Value));
                    ...
                    ctx.TB_DatosGralReporte.Add(new TB_DatosGralReporte()
                    {
                        ...
                        // se agregan estos para no ingresar mas datos a la tabla 
                        otrosH = new int?(this.txtHindigenas.Value == "" ? 0 : Convert.ToInt32(this.txtHindigenas.Value)),
                        otrosm = new int?(this.txtMindigenas.Value == "" ? 0 : Convert.ToInt32(this.txtMindigenas.Value))
                ...
                tbAuditoria entity = new tbAuditoria()
                {
                    ...
                    Descripcion = "Se edito el registro de reporte de actividades ,Construcción de una Cultura Incluyente",
                ...
                Response.Redirect("~/VistasReportes/Informe_Construccion_Cultura_Incluyente.aspx");
            }
            catch (Exception ex)
            {
        ...
        protected void lnkbtnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (file.HasFile)
            {
                foreach (HttpPostedFile postedFile in (IEnumerable<HttpPostedFile>)this.file.PostedFiles)
                {
                    ...
                    //if ( ValidateVideoExtension(fileName) == false)
            ...
            else
            {
                ...
                try
                {
                    ...
                    tbAuditoria audit = new tbAuditoria()
                    {
                        ...
                        PagModificacion = this.Page.Title,
                        Descripcion = "Nuevo registro de reporte de actividades ,Construcción de una Cultura Incluyente ",
                        usuario = this.txtResponsable.Text,
                    ...
                    #region metod viejo de guardado de foto
                    //if (this.file.HasFile)
                    //{
                    //    foreach (HttpPostedFile postedFile in (IEnumerable<HttpPostedFile>)this.file.PostedFiles)
                    //    {
                    ...
                    //                System.Drawing.Image ImagenOriginal = (Bitmap)((new ImageConverter()).ConvertFrom(numArray));

                    //                //crear imagen 
                    //                System.Drawing.Image imtThumbnail;
                    //                int TamnioThumbnail = 500;
                    //                imtThumbnail = RedimencionarImagen(ImagenOriginal, TamnioThumbnail);
                    //                byte[] bImagenThumbnail = new byte[TamnioThumbnail];

                    //                ImageConverter Convertidor = new ImageConverter();
                    //                bImagenThumbnail = (byte[])Convertidor.ConvertTo(imtThumbnail, typeof(byte[]));
                    //                string base64String = Convert.ToBase64String(bImagenThumbnail);
