...
{
    public partial class EscuelaSegura : System.Web.UI.Page
    {
        int personalID;
        int TotalAtendidos;
        int nino;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (User.IsInRole("SYSADMIN") || User.IsInRole("Foráneo") || User.IsInRole("Administrador") || User.IsInRole("CapEscolar") || User.IsInRole("Admin_Escolar"))
                {
            ...
            if (Session["idExpediente_Accion"] != null && Session["AccionExpediente"] != null)
            {
                ...
                switch ((int)accion)
                {
                    case (int)AccionExpediente.Creacion:
                        if (!User.IsInRole("SysAdmin") && !User.IsInRole("Foraneo") && !User.IsInRole("CapEscolar") && !User.IsInRole("Admin_Escolar"))
                        {
                            Response.Redirect("~/VistasReportes/InformeEscuelaSegura.aspx");
                        }
                        break;
        ...
        protected void expediente_Llenar(Guid idExpediente)
        {
            ...
            if (datosGralReporte != null)
            {
                ...
                this.ddlNivel.Text = datosGralReporte.Nivel != "0" ? datosGralReporte.Nivel : "0";
                this.ddlTurno.Text = datosGralReporte.Turno != "0" ? datosGralReporte.Turno : "0";
        ...
        #region ### SUMATORIA DE BENEFICIADOS
        //protected void txtnino_TextChanged(object sender, EventArgs e)
        //{
        //    CalcularTotalAlumnos();
        //    txtnino.Focus();
        //}

        //protected void txtnina_TextChanged(object sender, EventArgs e)
        //{
        //    CalcularTotalAlumnos();
        //    txtnina.Focus();
        //}

        //protected void txthombres_TextChanged(object sender, EventArgs e)
        //{
        //    CalcularTotalAlumnos();
        //    txthombres.Focus();
        //}

        //protected void txtmujeres_TextChanged(object sender, EventArgs e)
        //{
        //    CalcularTotalAlumnos();
        //}

        //protected void txtdocenteh_TextChanged(object sender, EventArgs e)
        //{
        //    CalcularTotalAlumnos();
        //}

        //protected void txtdocentem_TextChanged(object sender, EventArgs e)
        //{
        //    CalcularTotalAlumnos();
        //}
        //protected void CalcularTotalAlumnos()
        //{
        //    try
        //    {
        //        TotalAtendidos = 0;
        ...
        //        if (this.txthombres.Text != "") { padresH = int.Parse(txthombres.Text); }
        //        if (this.txtmujeres.Text != "") { padresM = int.Parse(txtmujeres.Text); }
        //        if (this.txtdocenteh.Text != "") { DocentesH = int.Parse(txtdocenteh.Text); }
        //        if (this.txtdocentem.Text != "") { DocentesM = int.Parse(txtdocentem.Text); }

        //        this.TotalAtendidos = nino + nina + padresH + padresM + DocentesH + DocentesM;
        ...
        protected void btnGuardarEdicion_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (tbReporteDiario != null)
                {
                    ...
                    int Hombres = nino + padresH + DocentesH;
                    int Mujeres = nina + padresM + DocentesM;
            ...
            else
            {
                ...
                try
                {
                    ...
                    tbAuditoria audit = new tbAuditoria()
                    {
                        ...
                        PagModificacion = this.Page.Title,
                        Descripcion = "Nuevo registro de reporte de actividades ,SEGURIDAD CIUDADANA Y PAZ SOCIAL EN ENTORNO EDUCATIVO",
                        usuario = this.txtResponsable.Text,
        ...
        [WebMethod(true)]
        public static List<ResumenDiario> Cordenadas(Guid ActividadId)
        {
            SIICOPEntities ctx = new SIICOPEntities();

            Guid idobtenido = ActividadId;

            List<ResumenDiario> carga = new List<ResumenDiario>();
            var cargar = (from a in ctx.tb_DireccionReporte
                          where a.idResumenDiario == idobtenido
                          select new
                          { a.Latitud, a.Longitud }).ToList();
            foreach (var obt in cargar)
            {
                carga.Add(new ResumenDiario
                {

                    Latitud = Convert.ToDecimal(obt.Latitud),
                    Longitud = Convert.ToDecimal(obt.Longitud)
                });
            }
