...
using OfficeOpenXml;
using SIICOP_V1._2.Captura.CVcMyCPC;
...
namespace SIICOP_V1._2.VistasReportes.CVcMyCPC
{
    public partial class Vista_de_datos_SESCESP : System.Web.UI.Page
    {
        ...
        protected void conteoactividadesAdmin()
        {
            ...
            {
                this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(ñ => ñ.Dependencia == "SESCESP").Count();
            ...
            else
            {
                ...
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.Dependencia == "SESCESP").Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => ((x.fecha >= fechainicial && x.fecha <= fechaFinal) && (x.Dependencia == "SESCESP") && (x.RegionID == Zona || x.programasID == tiprograma))).Count();
        ...
        protected void GvPCAdmin_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (valor == 27)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/CVcMyCPC/Pines/Enlace.png";
                    }
                    if (valor == 28)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/CVcMyCPC/Pines/VinculacionI.png";
                    }
        ...
        protected void GvPCAdmin_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Editar")
            {
                ...
                if (ENLACE == Progrmaid)
                {
                    this.Session["AccionReporte"] = Reporte_Actividades_SESCESP.AccionReporte.Edicion;
                    this.Session["idReporte_Accion"] = (object)idExped;
                    this.Response.Redirect("~/Captura/CVcMyCPC/Reporte_Actividades_SESCESP.aspx");
                }

                if (VINCULA == Progrmaid)
                {
                    this.Session["AccionReporte"] = Reporte_Actividades_SESCESP.AccionReporte.Edicion;
                    this.Session["idReporte_Accion"] = (object)idExped;
                    this.Response.Redirect("~/Captura/CVcMyCPC/Reporte_Actividades_SESCESP.aspx");
        ...
        private void LlenaHoja(OfficeOpenXml.ExcelWorksheet xlWorkSheetGral)
        {
            try
            {
                ...
                if (Session["dtPrincipal"] == null)
                {
                    ...
                    if (tiprograma == 0)
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "SESCESP"))).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
                    else
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "SESCESP") && R.programasID == tiprograma)).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
        ...
        public void ExportaImagenesZip()
        {
            ...
            if (programa == 0)
            {
                var consulta = (from Item1 in ctx.tb_Reporte_Diario
                                ...
                                where Item1.fecha >= fechainicial && Item1.fecha <= fechaFinal && Item3.Dependencia == "SESCESP"
                                select new
                                {
