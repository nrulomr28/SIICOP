...
namespace SIICOP_V1._2.VistasReportes.C4
{
    public partial class Vista_de_datos_de_actividades_C4 : System.Web.UI.Page
    {
        ...
        protected void conteoactividadesAdmin()
        {
            ...
            {
                this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(ñ => ñ.Dependencia == "C4").Count();
            ...
            else
            {
                ...
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.Dependencia == "C4").Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.Wv_DatosReportesHistorico.Where(x => ((x.fecha >= fechainicial && x.fecha <= fechaFinal) && (x.Dependencia == "C4") && (x.RegionID == Zona || x.programasID == tiprograma))).Count();
        ...
        #region ## METODO PARA EL MAPA DE GOOGLE 
        ...
        private void LlenaHoja(OfficeOpenXml.ExcelWorksheet xlWorkSheetGral)
        {
            try
            {
                ...
                if (Session["dtPrincipal"] == null)
                {
                    ...
                    if (tiprograma == 0)
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "C4"))).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
                    else
                    {
                        List<Wv_DatosReportesHistorico> listaReportes = ctx.Wv_DatosReportesHistorico.Where(R => (((convertidoFI && R.fecha >= fechaInicial) || !convertidoFI) && ((convertidoFF && R.fecha <= fechaFinal) || !convertidoFF) && (R.Dependencia == "C4") && R.programasID == tiprograma)).ToList();
                        dtPrincipal = ConvertToDataTable(listaReportes);
                    }
        ...
        protected void GvPCAdmin_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (valor == 16)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/C4/Pines/Pin_Escolar.png";
                    }
                    if (valor == 17)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/C4/Pines/Pin_Empresarial.png";
                    }
                    if (valor == 18)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/C4/Pines/IP.png";
                    }
                    if (valor == 19)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/C4/Pines/Pin_RedesVecinales.png";
                    }
                    if (valor == 20)
                    {
                        img.ImageUrl = "~/Imagenes/Banner/C4/Pines/Pin_Eventos.png";
                    }
        ...
        public void ExportaImagenesZip()
        {
            ...
            if (programa == 0)
            {
                var consulta = (from Item1 in ctx.tb_Reporte_Diario
                                ...
                                where Item1.fecha >= fechainicial && Item1.fecha <= fechaFinal && Item3.Dependencia == "C4"
                                select new
                                {
        ...
        protected void GvPCAdmin_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Editar")
            {
                ...
                {
                    this.Session["AccionReporte"] = Ficha_promocion_difusion.AccionReporte.Edicion;
