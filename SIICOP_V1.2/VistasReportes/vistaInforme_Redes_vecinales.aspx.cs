...
{
    public partial class vistaInforme_Redes_vecinales : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (this.User.IsInRole("SysAdmin") || this.User.IsInRole("Cap_PC") || (this.User.IsInRole("Admin_PC")))
                {
                    if (this.User.IsInRole("SysAdmin") || this.User.IsInRole("Admin_PC"))
                    {
                        this.GVEPartiCiudadano.Visible = false;
                        this.GvPCAdmin.Visible = true;
                    ...
                    else
                    {
                        ...
                        this.GvPCAdmin.Visible = false;
                        this.GVEPartiCiudadano.Visible = true;
        ...
        protected void conteoactividadesAdmin()
        {
            ...
            if (this.txtFInicialC.Text == string.Empty)
            {
                if (this.iduser != 0)
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.programasID == 8 && x.Personalid == this.iduser).Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.programasID == 8).Count();
            ...
            else
            {
                ...
                if (this.iduser != 0)
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.programasID == 8 && x.Personalid == this.iduser).Count();

                }
                else
                {
                    this.lblTotalRegistros.Text = "Registros:" + ctx.tb_Reporte_Diario.Where(x => x.fecha >= fechainicial && x.fecha <= fechaFinal && x.programasID == 8).Count();
        ...
        protected void GVEPartiCiudadano_DataBound(object sender, EventArgs e)
        {
            try
            {

                // Recupera la el PagerRow...
                GridViewRow pagerRow = GVEPartiCiudadano.BottomPagerRow;
                ...
                if ((pageList != null))
                {
                    ...
                    int i = 0;
                    for (i = 0; i <= GVEPartiCiudadano.PageCount - 1; i++)
                    {
                        // Se crea un objeto ListItem para representar la �gina...
                        int pageNumber = i + 1;
                        ListItem item = new ListItem(pageNumber.ToString());
                        if (i == GVEPartiCiudadano.PageIndex)
                        {
                            item.Selected = true;
                        }
                ...
                if ((pageLabel != null))
                {
                    ...
                    int currentPage = GVEPartiCiudadano.PageIndex + 1;
                    // Actualiza el Label control con la �gina actual.
                    pageLabel.Text = "Página " + currentPage.ToString() + " de " + GVEPartiCiudadano.PageCount.ToString();
        ...
        protected void GVEPartiCiudadano_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Editar")
            {
                ...
                if (Guid.TryParse(e.CommandArgument.ToString(), out numFila))
                {
                    ...
                    Response.Redirect("~/Captura/Redes_vecinales.aspx");
                }
            }
        ...
        {
            {
                ...
                GVEPartiCiudadano.PageIndex = pageList.SelectedIndex;
        ...
        #region Norte *********************************
        public class datoGraficaPasteNorte_redes
        {
            public int Cantidad_redes { get; set; }
            public string etiqueta_R { get; set; }
            public datoGraficaPasteNorte_redes() { }

        }

        [WebMethod(true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static List<datoGraficaPasteNorte_redes> ChartDataNorteAccionesta_redes(string x, string fechaInicial, string fechafinal)
        {
            SIICOPEntities ctx = new SIICOPEntities();
            ...
            try
            {
                List<datoGraficaPasteNorte_redes> listaDatos = new List<datoGraficaPasteNorte_redes>();

                datoGraficaPasteNorte_redes nuevoDG = new datoGraficaPasteNorte_redes();
                ...
                var resultado = (from ar in ctx.tb_Reporte_Diario
                                 ...
                                 where ar.fecha >= fechainicial && ar.fecha <= fechaFinal && xr.RegionID == 1 && ar.programasID == 8
                ...
                {
                    nuevoDG = new datoGraficaPasteNorte_redes();

                    nuevoDG.Cantidad_redes = data.AccionesID;
                    nuevoDG.etiqueta_R = data.AccionesNombre;
        ...
        public class datoGraficabarteNorte_redes
        {
            public int CantidadMuni_redes { get; set; }
            public string Municipios_redes { get; set; }
            public datoGraficabarteNorte_redes() { }

        }

        [WebMethod(true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static List<datoGraficabarteNorte_redes> datoGraficabarteNorteMuni_redes(string x, string fechaInicial, string fechafinal)
        {
            SIICOPEntities ctx = new SIICOPEntities();
            ...
            try
            {
                List<datoGraficabarteNorte_redes> listaDatos = new List<datoGraficabarteNorte_redes>();

                datoGraficabarteNorte_redes nuevoDG = new datoGraficabarteNorte_redes();
                ...
                foreach (var data in resultado)
                {
                    nuevoDG = new datoGraficabarteNorte_redes();

                    nuevoDG.CantidadMuni_redes = data.municipiosid;
                    nuevoDG.Municipios_redes = data.Municipios;
                    listaDatos.Add(nuevoDG);
                }
                return listaDatos.ToList();
        ...
        #region centro ***********************************
        public class datoGraficaPastecentro_redes
        {
            public int Cantidadcentro_redes { get; set; }
            public string etiquetaRcentro_redes { get; set; }
            public datoGraficaPastecentro_redes() { }

        }

        [WebMethod(true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static List<datoGraficaPastecentro_redes> ChartDataCentroAccionesta(string x, string fechaInicial, string fechafinal)
        {
            SIICOPEntities ctx = new SIICOPEntities();
            ...
            try
            {
                List<datoGraficaPastecentro_redes> listaDatos = new List<datoGraficaPastecentro_redes>();

                datoGraficaPastecentro_redes nuevoDG = new datoGraficaPastecentro_redes();
                ...
                var resultado = (from bc in ctx.tb_Reporte_Diario
                                 ...
                                 where bc.fecha >= fechainicial && bc.fecha <= fechaFinal && nb.RegionID == 2 && bc.programasID == 8
                ...
                foreach (var data in resultado)
                {
                    nuevoDG = new datoGraficaPastecentro_redes();

                    nuevoDG.Cantidadcentro_redes = data.AccionesID;
                    nuevoDG.etiquetaRcentro_redes = data.AccionesNombre;
                    listaDatos.Add(nuevoDG);
                }
                return listaDatos.ToList();
        ...
        public static List<datoGraficabartecentro> datoGraficabarteCentroMuni(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from arx in ctx.tb_Reporte_Diario
                                 ...
                                 where arx.fecha >= fechainicial && arx.fecha <= fechaFinal && xry.RegionID == 2 && arx.programasID == 8
        ...
        public static List<datoGraficaPasteSur> ChartDataSurAccionesta(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from bxc in ctx.tb_Reporte_Diario
                                 ...
                                 where bxc.fecha >= fechainicial && bxc.fecha <= fechaFinal && nb.RegionID == 3 && bxc.programasID == 8
        ...
        public static List<datoGraficabartesur> datoGraficabartesurMuni(string x, string fechaInicial, string fechafinal)
        {
            ...
            try
            {
                ...
                var resultado = (from xxx in ctx.tb_Reporte_Diario
                                 ...
                                 where xxx.fecha >= fechainicial && xxx.fecha <= fechaFinal && rrr.RegionID == 3 & xxx.programasID == 8
        ...
        protected void GVEPartiCiudadano_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        private void Conteo_A_B_TB_X_Cargadetrabajo()
        {
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjXAL = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION XALAPA XX" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtXAL.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION XALAPA XX" && s.programasID == 8
            ...
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJXAL = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION XALAPA XX" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjVER = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION VERACRUZ XXIII" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtVER.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION VERACRUZ XXIII" && s.programasID == 8
                                                     select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJVER = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION VERACRUZ XXIII" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjPOZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION POZA RICA" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtPOZA.InnerText = (from s in ctx.tb_Reporte_Diario
                                                      join std in ctx.tb_DireccionReporte
                                                      on s.idResumenDiario equals std.idResumenDiario
                                                      where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "CONURBACION POZA RICA" && s.programasID == 8
                                                      select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJPOZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "CONURBACION POZA RICA" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjCOR = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION CORDOBA" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjTCOR.InnerText = (from s in ctx.tb_Reporte_Diario
                                                     join std in ctx.tb_DireccionReporte
                                                     on s.idResumenDiario equals std.idResumenDiario
                                                     where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "COORDINACION CORDOBA" && s.programasID == 8
                                                     select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJCOR = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION CORDOBA" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjCOATZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION COATZACOALCOS" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtCOATZA.InnerText = (from s in ctx.tb_Reporte_Diario
                                                        join std in ctx.tb_DireccionReporte
                                                        on s.idResumenDiario equals std.idResumenDiario
                                                        where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "COORDINACION COATZACOALCOS" && s.programasID == 8
                                                        select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJCOATZA = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "COORDINACION COATZACOALCOS" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
            ...
            //ACCIONES
            tbCarga_trabajo_Dele AccionesXareaObjENLACE = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "Enlace" && x.Tipo_TA_BA_TB == 1).FirstOrDefault();
            ...
            this.spanAccionXareaObjtENLACE.InnerText = (from s in ctx.tb_Reporte_Diario
                                                        join std in ctx.tb_DireccionReporte
                                                        on s.idResumenDiario equals std.idResumenDiario
                                                        where s.fecha.Value.Month >= mesactual && s.DelegacionOcoonurbacion == "ENLACES" && s.programasID == 8
                                                        select s.programasID).Count().ToString();
            ...
            //CONTEO DE BENEFICIADOS POR MES 
            tbCarga_trabajo_Dele beneficiadosGralxmesdOBJENLACE = this.ctx.tbCarga_trabajo_Dele.Where(x => x.programa == 8 && x.Mes_Num == mesactual && x.Delegacion == "Enlace" && x.Tipo_TA_BA_TB == 3).FirstOrDefault();
