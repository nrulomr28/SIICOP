...
{
    public partial class SectorEmpresarial : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (User.IsInRole("SYSADMIN") || User.IsInRole("Foráneo") || User.IsInRole("Administrador") || User.IsInRole("CapSEMP") || User.IsInRole("Admin_Empresarial"))
                {
            ...
            if (Session["idExpediente_Accion"] != null && Session["AccionExpediente"] != null)
            {
                ...
                switch ((int)accion)
                {
                    case (int)AccionExpediente.Creacion:
                        if (!User.IsInRole("SysAdmin") && !User.IsInRole("Foraneo") && !User.IsInRole("CapSEMP") && !User.IsInRole("Admin_Empresarial"))
                        {
                            Response.Redirect("~/VistasReportes/Info_Enlace_con_el_Sector_Empresarial.aspx");
                        }
                        break;
        ...
        protected void btnGuardarEdicion_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                tbAuditoria entity = new tbAuditoria()
                {
                    ...
                    Descripcion = "Se edito el registro de reporte de actividades ,Enlace Sector Empresarial",
        ...
        protected void lnkbtnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (file.HasFile)
            {
              ...
              //  int tamanoarchivo = postedFile.ContentLength;

             //   int limiteMaximo = 3 * 1024 * 1024; // 3,145,728 bytes

                //tamanoarchivo = Server.HtmlDecode.postedFile.ContentLength;
            //    if (tamanoarchivo > limiteMaximo)
             //   {
            //       textoValidacion += $"<li>El tamaño máximo permitido por archivo es de 3 MB. (Tu archivo pesa: {tamanoArchivo / 1024 / 1024} MB)</li>";
              //      valido = false;
               // }
            ...
            if(ddlAcciones.SelectedValue == "1065" || ddlAcciones.SelectedValue == "1158")
            {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        NuevoRegistros.TotalHombresAtendidos = 0;
                        NuevoRegistros.TotalMujeresAtendidas = 0;
                        NuevoRegistros.total_atendidos = 0;
                    }
                    ...
                    tbAuditoria audit = new tbAuditoria()
                    {
                        ...
                        PagModificacion = this.Page.Title,
                        Descripcion = "Nuevo registro de reporte de actividades ,Enlace con el Sector Empresarial",
                        usuario = this.txtResponsable.Text,
        ...
        {
            if (ddlAcciones.SelectedValue == "1158" || ddlAcciones.SelectedValue == "1065")
            {
                atendidos.Visible = true;
            }
            else
            {
                atendidos.Visible = false;
            }
        }
    }
}